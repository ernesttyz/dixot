function DIXOTreducedB(testprob,isprojinput)

%The input <testprob> specifies the testing probability
%The input <isprojinput> specifies whether sequential measurements are assumed to be projective (however, it still assumes measurement operators are hermitian)
%This code omits one projector per measurement, using the normalization conditions to eliminate it from all constraints and/or objective functions
%Estimated runtime 4hrs per run

global sizeA sizeB sizeX isproj
rng(1); 
tic;
isproj = isprojinput
...
sizeA=4;sizeB=4;sizeX=3;sizeY=7; %We will order Bob's measurements as [Mtest0 Mtest1 Mtest2 Mprot0 Mprot1 Mprot2 Mmessage]
...
maxA=sizeA-1;maxB=sizeB-1; %Omit one projector per measurement via normalization conditions
for x = 1:sizeX
    for a = 1:maxA
...
% Extra moments for sequential protocol measurements, appended at the end of opslistB
for yprot=4:6
    for b = 1:sizeB-1 %Bob's first message
    for g = 1:sizeB-1 %Bob's guess
        opslistB{end+1}=optype([g,b; yprot+sizeX,7+sizeX]);
    end
    end
end
size(opslistA)
size(opslistB)
size(celluniques(opslistB))
...
numopscheck = length(celluniques(opslist))
...
% (2) Squared projectors should be collapsed
...
if mod(j,30)==0 %Print timestamp every 30 rows
    j
    datetime('now')
end
end
constr=[constr,momentsmat>=0];
...
cheatterm={{ [1 1 1 1; 1 [7 4 7]+sizeX*ones(1,3)] [1 2 1 2; 1 [7 4 7]+sizeX*ones(1,3)] [1 1 1 1; 2 [7 5 7]+sizeX*ones(1,3)] [1 3 1 3; 2 [7 5 7]+sizeX*ones(1,3)] [1 2 1 2; 3 [7 6 7]+sizeX*ones(1,3)] [1 3 1 3; 3 [7 6 7]+sizeX*ones(1,3)] }, ones(1,6)/3 };
cheatterm=reduceprojs(cheatterm);
for x=1:3
for a=2:4
for b=1:4
    cheatterm = lincombplus(cheatterm, reduceprojs({{ [a b a b; x [7 x+3 7]+sizeX*ones(1,3)] }, [1/3]} ));
end
end
x
datetime('now')
...
testvars=0; testterm = reduceprojs(testterm);
...
cheatvars=0; %cheatterm = reduceprojs(cheatterm);
...
opsnew = celluniques([ops1 ops2]); 
%%%%%%%%%%%%%%%%%% The part between these lines makes lincombplus always delete orthogonal projectors. Comment this out if that is not desired.
for j=length(opsnew):-1:1 %Has to go backwards because items are being deleted from the array
    if isequal(optype(opsnew{j}),0)==1
        opsnew(j)=[];
    end
end
%%%%%%%%%%%%%%%%%%
coeffsnew=zeros(1,length(opsnew)); nonzeros=[];
...
for j=1:size(temp,2)-1
    ...
    global sizeX isproj 
    if temp1(2)==temp2(2) && temp1(1)~=temp2(1) && temp2(2)~=isproj*(7+sizeX) 
        temp=0;
        break
    end
end
...
for loopcounter=1:100 %We'll bound it at maximum 100 iterations
    ...
    for j=size(collapsed,2)-1:-1:1 %Warning: using length(collapsed') causes problems when there is only one projector in opmatrix, because in that case opmatrix is only 2x1 and length() gives the number of rows rather than columns.
        if isequal(collapsed(:,j),collapsed(:,j+1)) && collapsed(2,j)~=isproj*(7+sizeX) 
            collapsed(:,j)=[];
            nosquares=0;
        end
    end
